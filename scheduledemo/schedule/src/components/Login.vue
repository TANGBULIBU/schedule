...
function tologin() {
  if (checkUsername() && checkUserPwd()) {
    axios.post("http://localhost:80/user/tologin", loginUser)
        .then(response => {
          ...
          if (response.data.message == '登录成功') {
            ...
            //将用户存储到localStorage
            window.localStorage.setItem("uname", loginUser.username)
            //将登录的uid存起
            window.localStorage.setItem("uid", response.data.data.uid)
            myrouter.push({path: "/showSchedule"})
          } else {
            //提示用户信息
    ...
    <table class="tab" cellspacing="0px">
      ...
      <tr class="ltr">
        <td colspan="2" class="buttonContainer">
          <input class="btn1" type="button" value="登录" @click="tologin()">
